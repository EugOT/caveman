...
function loadNullclawHelper() {
  try {
    return require(path.join(__dirname, '..', '..', 'bin', 'lib', 'nullclaw.js'));
  } catch (_) { return null; }
}
...
const AGENTS = [
  ...
  { id: 'agents',   file: 'AGENTS.md',
    frontmatter: '',
    mode: 'append',
    aliases: ['antigravity', 'antigravity-app', 'antigravity-cli', 'claude', 'claude-code',
      'claude-desktop', 'codex', 'codex-app', 'codex-cli', 'hermes', 'pi', 'pz',
      'warp', 'warp-preview', 'warppreview'] },
  ...
  { id: 'codex-skill', file: '.codex/skills/caveman/SKILL.md',
    mode: 'skill',
    default: false,
    aliases: ['codex', 'codex-app', 'codex-cli', 'claw', 'goclaw', 'walcode', 'walkode', 'zeroclaw'] },
  { id: 'claude-skill', file: '.claude/skills/caveman/SKILL.md',
    mode: 'skill',
    default: false,
    aliases: ['claude', 'claude-code', 'claude-desktop'] },
  { id: 'pi-skill', file: '.pi/skills/caveman/SKILL.md',
    mode: 'skill',
    default: false,
    aliases: ['pi'] },
  { id: 'pz-skill', file: '.pz/skills/caveman/SKILL.md',
    mode: 'skill',
    default: false,
    aliases: ['pz'] },
  { id: 'claw', file: '.claw/instructions.md',
    frontmatter: '',
    mode: 'append',
    default: false,
  ...
  { id: 'openclaw', description: '~/.openclaw/workspace/{skills/caveman/, SOUL.md}',
    installer: 'openclaw' },
  { id: 'nullclaw', description: '~/.nullclaw/workspace/skills/caveman/SKILL.md',
    installer: 'nullclaw',
    default: false },
];
...
function loadSkillBody() {
  try {
    const local = path.join(__dirname, '..', '..', 'skills', 'caveman', 'SKILL.md');
    if (fs.existsSync(local)) return fs.readFileSync(local, 'utf8').trimEnd() + '\n';
  } catch (e) {}
  return [
    '---',
...
function processAgent(agent, targetDir, ruleBody, opts) {
  ...
  if (agent.mode === 'append') {
    if (!opts.dryRun) {
      ...
      writeFileSafe(fullPath, existing + sep + ruleBody, targetDir);
    }
...
function processNullclaw(opts) {
  ...
  if (!helper) {
    return {
      status: 'unsupported-standalone',
      label: 'x',
      detail: '~/.nullclaw/workspace (helper unavailable in standalone curl|node mode — use `npx -y github:JuliusBrussee/caveman -- --only nullclaw`)',
    };
  }
  const repoRoot = path.resolve(__dirname, '..', '..');
  const log = {
  ...
  if (!r.ok) {
    return { status: 'skipped-' + (r.reason || 'failed'), label: '?', detail: helper.resolveWorkspace ? helper.resolveWorkspace() : '~/.nullclaw/workspace' };
  }
  if (r.dryRun) return { status: 'would-add', label: '+', detail: helper.resolveWorkspace() };
  return { status: 'installed', label: '+', detail: helper.resolveWorkspace() };
}
...
function resolveAgents(onlyIds) {
  ...
  for (const raw of onlyIds) {
    ...
    const matches = AGENTS.filter(agent => agent.id === id || (agent.aliases || []).includes(id));
    if (matches.length === 0) {
      unknown.push(raw);
      continue;
    }
  ...
  if (unknown.length) {
    const valid = AGENTS.flatMap(agent => [agent.id, ...(agent.aliases || [])]).sort();
    throw new Error(`unknown agent: ${unknown.join(', ')}\nvalid ids: ${[...new Set(valid)].join(', ')}`);
  }
  return out;
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--dry-run') opts.dryRun = true;
      ...
      const v = argv[++i];
      if (!v || v.startsWith('--')) throw new Error('--only requires an agent id');
      opts.only.push(v);
    }
...
${AGENTS.map(a => {
  ...
  const aliases = a.aliases && a.aliases.length ? ` aliases: ${a.aliases.join(', ')}` : '';
...
module.exports = { processAgent, loadRuleBody, loadSkillBody, resolveAgents, AGENTS, SENTINEL, RULE_BODY };
