...
//   ├── package.json
//   ├── plugin.js              ← this file
//   └── caveman-config.cjs     ← bundled flag-helper shim (canonical source:
//                                  src/plugins/opencode/caveman-config.cjs)
//
...
// caveman-config.cjs sits next to plugin.js — it is the IRREDUCIBLE opencode
// shim's bundled flag/config helpers (symlink-safe safeWriteFlag, readFlag,
// getDefaultMode, VALID_MODES). opencode mandates a JS ESM plugin, so the JS
// flag logic it needs lives here as a co-located .cjs sibling rather than as a
// Zig binary call on opencode's per-message hot path. After the pure-Zig cutover
// (R6.4) this .cjs is the canonical source for the opencode path — there is no
// longer a src/hooks/caveman-config.js to fall back to. (The .cjs extension is
// required because this directory's package.json declares "type": "module", so
// a bare .js sibling would be parsed as ESM.)
//
// Loaded by evaluating the file as CommonJS by hand, NOT via the module loader:
// opencode runs plugins inside a compiled Bun binary where require() of on-disk
// files is rejected ("require() async module is unsupported") and await
// import() of a CJS file yields an empty namespace — both silently break the
// plugin (#418 follow-up). createRequire() still resolves node BUILT-INS fine in
// the compiled binary, which is all caveman-config needs (fs/path/os).
function loadConfig() {
