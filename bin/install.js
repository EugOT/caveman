...
function parseArgs(argv) {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (a.startsWith("--with-mcp-shrink=")) {
			...
			if (tokens.length === 0) {
				die(
					"error: --with-mcp-shrink requires an upstream command\n" +
		...
		switch (a) {
			...
			case "--with-mcp-shrink": {
				...
				if (v && !v.startsWith("--")) {
					...
					die(
						"error: --with-mcp-shrink requires an upstream command — caveman-shrink\n" +
			...
			case "--only": {
				...
				if (INIT_TARGET_ALIASES.has(id) || INIT_ONLY_AGENTS.has(id))
					opts.initOnly.push(id);
				break;
			}
			case "--config-dir": {
				const v = argv[++i];
	...
	if (opts.only.length) {
		...
		for (const id of opts.only) {
			...
			const isInitTarget =
				INIT_TARGET_ALIASES.has(id) || INIT_ONLY_AGENTS.has(id);
			...
			if (!isProvider && isInitTarget && !opts.withInit) {
				die(
					`error: ${id} is a repo-local init target; pass --with-init or --all`,
				);
			}
		}
	}
...
function checkWslWindowsNode() {
	...
	if (process.env.WSL_DISTRO_NAME) {
		die(
			"caveman: detected Windows Node.js running inside WSL.\n" +
	...
	try {
		...
		if (v.includes("microsoft") || v.includes("wsl")) {
			die(
				"caveman: detected Windows Node.js running inside WSL (/proc/version).\n" +
					"         Install Linux-native Node inside your WSL distro and re-run there.",
			);
		}
	} catch (_) {
		/* /proc/version absent on real Windows — fine */
	}
}
...
function checkNodeVersion() {
	...
	if (major < 18)
		die(
			`caveman: Node ${process.versions.node} too old. Need Node ≥18. https://nodejs.org`,
		);
}
...
const PROVIDERS = [
	...
	},
	{
		id: "gemini",
		label: "Gemini CLI",
		mech: "gemini extensions install",
		detect: "command:gemini",
	},
	{
		id: "opencode",
		label: "opencode",
		mech: "native opencode plugin",
		detect: "command:opencode",
	},
	{
		id: "openclaw",
		label: "OpenClaw",
		mech: "workspace skill + SOUL.md",
		detect: "command:openclaw||dir:$HOME/.openclaw/workspace",
	},
	{
		id: "nullclaw",
		label: "NullClaw",
		mech: "workspace skill",
		detect: "command:nullclaw",
	},
	{
		id: "codex",
		label: "Codex CLI",
		mech: "npx skills add (codex)",
	...
	},
	{
		id: "continue",
		label: "Continue",
		mech: "npx skills add (continue)",
		detect: "vscode-ext:continue.continue||vscode-ext:continue",
		profile: "continue",
	},
	{
		id: "kilo",
		label: "Kilo Code",
		mech: "npx skills add (kilo)",
	...
	// because Copilot ships as an editor extension, not a CLI (issue #336).
	{
		...
		mech: "npx skills add (github-copilot)",
		detect:
			"vscode-ext:github.copilot||vscode-ext:github.copilot-chat||cursor-ext:github.copilot",
	...
	// behind on uninstall).
	{
		id: "aider-desk",
		label: "Aider Desk",
		mech: "npx skills add (aider-desk)",
	...
	},
	{
		id: "crush",
		label: "Crush",
		mech: "npx skills add (crush)",
		detect: "command:crush",
		profile: "crush",
	},
	{
		id: "devin",
		label: "Devin (terminal)",
		mech: "npx skills add (devin)",
		detect: "command:devin",
		profile: "devin",
	},
	{
		id: "droid",
		label: "Droid (Factory)",
		mech: "npx skills add (droid)",
		detect: "command:droid",
		profile: "droid",
	},
	{
		id: "forgecode",
		label: "ForgeCode",
		mech: "npx skills add (forgecode)",
		detect: "command:forge",
		profile: "forgecode",
	},
	{
		id: "goose",
		label: "Block Goose",
		mech: "npx skills add (goose)",
		detect: "command:goose",
		profile: "goose",
	},
	{
		id: "iflow",
		label: "iFlow CLI",
		mech: "npx skills add (iflow-cli)",
		detect: "command:iflow",
		profile: "iflow-cli",
	},
	{
		id: "kiro",
		label: "Kiro CLI",
		mech: "npx skills add (kiro-cli)",
		detect: "command:kiro",
		profile: "kiro-cli",
	},
	{
		id: "mistral",
		label: "Mistral Vibe",
		mech: "npx skills add (mistral-vibe)",
		detect: "command:mistral",
		profile: "mistral-vibe",
	},
	{
		id: "openhands",
		label: "OpenHands",
		mech: "npx skills add (openhands)",
		detect: "command:openhands",
		profile: "openhands",
	},
	{
		id: "qwen",
	...
	},
	{
		...
		mech: "npx skills add (rovodev)",
		detect: "command:rovodev",
		profile: "rovodev",
	},
	{
		id: "tabnine",
		label: "Tabnine CLI",
		mech: "npx skills add (tabnine-cli)",
		detect: "command:tabnine",
		profile: "tabnine-cli",
	},
	{
		id: "trae",
		label: "Trae",
		mech: "npx skills add (trae)",
		detect: "command:trae",
		profile: "trae",
	},
	{
	...
	//   gemini CLI on first use — not a reliable signal of antigravity itself.
	{
		...
		mech: "npx skills add (junie)",
		detect: "jetbrains-plugin:junie",
		profile: "junie",
		soft: true,
	},
	{
	...
	},
	{
		...
		mech: "npx skills add (antigravity)",
...
function cursorExtPresent(needle) {
	...
	const re = new RegExp(needle, "i");
	try {
		return fs.readdirSync(dir).some((e) => re.test(e));
	} catch (_) {
		return false;
	}
}
...
function jetbrainsPresent() {
	const home = os.homedir();
	return (
		fs.existsSync(path.join(home, "Library/Application Support/JetBrains")) ||
	...
	for (const clause of spec.split("||")) {
		...
		switch (kind) {
			case "command":
				...
				break;
			case "dir":
				ok = safeStat(val, "isDirectory");
				break;
			case "file":
				ok = safeStat(val, "isFile");
				break;
			case "macapp":
				ok = macAppPresent(val);
				break;
			case "vscode-ext":
				ok = vscodeExtPresent(val);
				break;
			case "cursor-ext":
				ok = cursorExtPresent(val);
				break;
			case "jetbrains-config":
				ok = jetbrainsPresent();
				break;
			case "jetbrains-plugin":
				ok = jetbrainsPluginPresent(val);
				break;
		}
		if (ok) return true;
	}
...
async function installClaude(ctx) {
	...
	// skip standalone wiring because the plugin manifest handles hooks.
	{
		...
		if (settings) {
			...
			if (pruned > 0) {
				note(
					`  removed ${pruned} orphaned caveman hook entr${pruned === 1 ? "y" : "ies"} from settings.json (target script missing)`,
				);
				if (!opts.dryRun) {
	...
	if (opts.withHooks === false) {
		...
		if (pluginInstallSucceeded) {
			warn(
				"  --with-hooks wires hooks in settings.json alongside the plugin manifest.",
			);
			warn(
				"  Both will fire on every event. Pass --no-hooks to keep only the plugin path.",
			);
		}
	} else {
		// 'auto'
		...
		if (!shouldWireHooks) {
			...
			note(
				"  (pass --with-hooks to also wire standalone hooks in settings.json)",
			);
			results.skipped.push(["claude-hooks", "plugin manifest handles hooks"]);
		} else {
			note(
				"  hooks: plugin install did not succeed; falling back to standalone wiring",
			);
		}
	}
	...
	if (shouldWireHooks) {
		...
		if (r === "ok") results.installed.push("claude-hooks");
			...
			results.skipped.push(["claude-hooks", "already wired"]);
...
function installGemini(ctx) {
	...
	const r = runSpawn(
		"gemini",
		["extensions", "install", `https://github.com/${REPO}`],
		null,
		opts.dryRun,
	);
...
function installOpencode(ctx) {
	...
	if (!repoRoot) {
		warn(
			"  opencode native install requires a local clone of the caveman repo.",
		);
		note(
			"  Re-run from a clone: git clone https://github.com/" +
				REPO +
				" && cd caveman && node bin/install.js --only opencode",
		);
		results.failed.push([
			"opencode",
			"native install requires local repo clone",
		]);
		process.stdout.write("\n");
		return;
	}
	...
	if (opts.dryRun) {
		note(
			`  would mkdir ${pluginDir}/, ${commandsDir}/, ${agentsDir}/, ${skillsDir}/`,
		);
		note(
			`  would copy plugin.js + package.json + caveman-config.cjs into ${pluginDir}/`,
		);
		note(
			`  would copy ${OPENCODE_COMMAND_FILES.length} command files into ${commandsDir}/`,
		);
		note(
			`  would copy ${OPENCODE_AGENT_FILES.length} cavecrew agents into ${agentsDir}/`,
		);
		note(
			`  would copy ${OPENCODE_SKILL_DIRS.length} skill dirs into ${skillsDir}/`,
		);
		note(
			`  would patch ${opencodeJson} with "plugin" entry${opts.withMcpShrink ? " + caveman-shrink MCP" : ""}`,
		);
	...
	try {
		...
		for (const f of OPENCODE_AGENT_FILES) {
			...
			fs.writeFileSync(
				dest,
				stripOpencodeAgentTools(fs.readFileSync(src, "utf8")),
			);
			process.stdout.write(`  installed: ${dest}\n`);
		}
		...
		const ruleBody =
			fs
				.readFileSync(
					path.join(repoRoot, "src", "rules", "caveman-activate.md"),
					"utf8",
				)
				.trimEnd() + "\n";
		...
		if (fs.existsSync(agentsMd)) {
			...
			const alreadyFenced =
				existing.includes(OPENCODE_AGENTS_MD_BEGIN) &&
				existing.includes(OPENCODE_AGENTS_MD_END);
			const alreadyByLegacySentinel =
				!alreadyFenced && existing.includes(OPENCODE_AGENTS_MD_SENTINEL);
			if (alreadyFenced) {
				note(`  ${agentsMd} already contains caveman ruleset`);
			} else if (alreadyByLegacySentinel) {
				note(
					`  ${agentsMd} contains a legacy (un-fenced) caveman block — leaving as-is`,
				);
				...
				if (opts.force) {
					...
					process.stdout.write(
						`  rewrote ${agentsMd} with fenced caveman block\n`,
					);
				}
			} else {
				const sep = existing.endsWith("\n\n")
					...
					: existing.endsWith("\n")
						? "\n"
						: "\n\n";
				fs.writeFileSync(agentsMd, existing + sep + fencedBlock, {
					mode: 0o644,
				});
		...
		if (cfg === null) {
			warn(
				`  ${opencodeJson} unparseable; will not touch it. Edit manually then re-run.`,
			);
		...
		if (fs.existsSync(opencodeJson) && !fs.existsSync(opencodeBak)) {
			try {
				fs.copyFileSync(opencodeJson, opencodeBak);
			} catch (_) {}
		}
		...
		if (opts.withMcpShrink) {
			...
			if (!cfg.mcp["caveman-shrink"]) {
				...
				process.stdout.write(
					`  registered caveman-shrink MCP server (wraps: ${opts.withMcpShrink.join(" ")})\n`,
				);
			}
		}
...
async function installHooks(ctx) {
	...
	if (opts.dryRun) {
		...
		for (const f of HOOK_FILES)
			note(`  would install ${path.join(hooksDir, f)}`);
		note(
			`  would merge SessionStart + UserPromptSubmit + statusline into ${settingsPath}`,
		);
		return "ok";
	}
	...
	for (const f of HOOK_FILES) {
		...
		if (sourceDir && fs.existsSync(path.join(sourceDir, f))) {
			...
			try {
				await downloadTo(`${HOOKS_REMOTE}/${f}`, dest);
			} catch (e) {
				return `download ${f} failed: ${e.message}`;
			}
			...
			if (checksums) {
				...
				if (!want || want !== got) {
					...
					return (
						`integrity check failed for ${f} (expected ${want || "<not in manifest>"}, got ${got}) — ` +
				...
				warn(
					`  note: no integrity manifest at ${PINNED_REF} — downloaded hooks installed unverified.`,
				);
			}
		}
		process.stdout.write(`  installed: ${dest}\n`);
	...
	try {
		fs.chmodSync(path.join(hooksDir, "caveman-statusline.sh"), 0o755);
	} catch (_) {}
	...
	if (settings === null) {
		warn(
			"  settings.json unparseable; will not touch it. Edit manually then re-run.",
		);
	...
	const psHost =
		IS_WIN && hasCmd("pwsh") ? "pwsh" : IS_WIN ? "powershell" : null;
	const slCmd = IS_WIN
	...
	if (!settings.statusLine) {
		...
		if (
			existing.includes(statusline) ||
			existing.includes("caveman-statusline")
		) {
			process.stdout.write("  statusline badge already configured.\n");
		} else {
			process.stdout.write(
				"  NOTE: existing statusline detected — caveman badge NOT added.\n",
			);
			process.stdout.write(
				"        See src/hooks/README.md to add the badge to your existing statusline.\n",
			);
		}
	}
...
function installMcpShrink(ctx) {
	...
	if (probe.status !== 0) {
		warn(
			`    'npm view ${MCP_SHRINK_PKG}' returned no metadata — registry unreachable or package missing.`,
		);
		note(
			"    Skipping registration. Re-run --with-mcp-shrink when the registry is reachable.",
		);
	...
	if (help.status !== 0) {
		note(
			"    'claude mcp add' not available on this CLI. Add the snippet from",
		);
	...
	if ((r.status || 0) === 0) {
		...
		note(
			`    Edit ~/.claude.json mcpServers["caveman-shrink"] to change the upstream,`,
		);
		note("    or `claude mcp remove caveman-shrink` to drop it.");
		note(
			`    Docs: https://github.com/${REPO}/tree/main/src/mcp-servers/caveman-shrink`,
		);
		return { kind: "ok" };
	}
...
async function runInit(ctx) {
	...
	if (opts.only.length && initOnly.length === 0) {
		note(
			"  no repo-local init target matches selected --only agent(s); skipping",
		);
		return true;
	}
...
function downloadTo(url, dest) {
	...
	if (hasCmd("curl")) {
		const r = child_process.spawnSync("curl", ["-fsSL", "-o", dest, url], {
			stdio: "inherit",
		});
...
async function loadRemoteHookChecksums() {
	const tmp = path.join(
		os.tmpdir(),
		`caveman-checksums-${process.pid}-${Date.now()}.sha256`,
	);
	try {
...
function uninstall(ctx) {
		...
		if (settings) {
			...
			if (!opts.dryRun) {
				try {
					SETTINGS.writeSettings(settingsPath, settings);
					ok(
						`  removed ${removed} caveman hook entr${removed === 1 ? "y" : "ies"} from settings.json`,
					);
				} catch (e) {
	...
	if (fs.existsSync(ocPluginDir)) {
		...
		if (!opts.dryRun) {
			try {
				fs.rmSync(ocPluginDir, { recursive: true, force: true });
			} catch (_) {}
		}
		...
		for (const f of OPENCODE_COMMAND_FILES) {
			...
			if (fs.existsSync(p) && !opts.dryRun) {
				try {
					fs.unlinkSync(p);
				} catch (_) {}
			}
		...
		if (fs.existsSync(ocAgentsMd)) {
			...
			if (begin !== -1 && end !== -1 && end > begin) {
				...
				const after = body
					.slice(end + OPENCODE_AGENTS_MD_END.length)
					.replace(/^\n+/, "\n");
				...
				if (
					...
					note(
						`  left ${ocAgentsMd} in place (legacy mixed content — strip caveman block manually)`,
					);
				}
			}
		}
		...
		if (fs.existsSync(ocFlag) && !opts.dryRun) {
			try {
				fs.unlinkSync(ocFlag);
			} catch (_) {}
		}
	}
	...
	if (
		fs.existsSync(path.join(ocwWs, "skills", "caveman")) ||
		fs.existsSync(path.join(ocwWs, "SOUL.md"))
	) {
		const log = {
			write: (s) => process.stdout.write(s),
			note: (s) => note(s),
		...
		const r = OPENCLAW.uninstallOpenclaw({
			workspace: ocwWs,
			dryRun: opts.dryRun,
			log,
		});
	...
	if (fs.existsSync(path.join(ncWs, "skills", "caveman"))) {
		...
		const r = NULLCLAW.uninstallNullclaw({
			workspace: ncWs,
			dryRun: opts.dryRun,
			log,
		});
	...
	ok("uninstall done.");
	ok(
		"npx-skills installs (Cursor/Windsurf/etc.) — remove via your IDE's skill manager",
	);
	ok(
		"per-repo init files (.cursor/, .windsurf/, AGENTS.md) — remove with your editor",
	);
}
...
async function promptForOnly(detected) {
	...
	const ans = await new Promise((res) =>
		rl.question("Install which? (default: all) ", res),
	);
	rl.close();
...
function printList(noColor) {
	...
	process.stdout.write(
		`  ${pad("ID", 13)} ${pad("AGENT", 22)} INSTALL MECHANISM\n`,
	);
	process.stdout.write(
		`  ${pad("--", 13)} ${pad("-----", 22)} -----------------\n`,
	);
	for (const p of PROVIDERS) {
		...
		process.stdout.write(
			`  ${pad(p.id, 13)} ${pad(p.label, 22)} ${p.mech}${tag}\n`,
		);
	}
	process.stdout.write("\n");
	process.stdout.write(
		c.dim(
			"  Aliases: aider→aider-desk, claude-code→claude, codex-app/codex-cli→codex,\n",
		),
	);
	process.stdout.write(
		c.dim(
			"           antigravity-app/antigravity-cli→antigravity, warpPreview/warp-preview→warp.\n",
		),
	);
	process.stdout.write(
		c.dim(
			"  Repo-only init ids: agents, claude-desktop, perplexity, zeroclaw, goclaw, hermes, pi, pz, walcode, walkode, claw.\n",
		),
	);
	process.stdout.write(
		c.dim(
			"  Defaults: --with-hooks ON, --with-init OFF, --with-mcp-shrink OFF.\n",
		),
	);
	process.stdout.write(
		c.dim(
			"  --all = hooks + init (mcp-shrink needs an upstream — opt in explicitly).\n",
		),
	);
	process.stdout.write(
		c.dim("  --minimal turns hooks + init + mcp-shrink off.\n"),
	);
}
...
async function main() {
	...
	if (
		...
		ctx.say(
			"→ no known agents detected — running npx-skills auto-detect fallback",
		);
		...
		const r = runSpawn(
			"npx",
			["-y", "skills", "add", REPO, "--yes", "--all"],
			null,
			opts.dryRun,
		);
		if ((r.status || 0) === 0) ctx.results.installed.push("skills-auto");
		else
			ctx.results.failed.push(["skills-auto", "npx skills add (auto) failed"]);
		process.stdout.write("\n");
	}
	...
	if (opts.withInit) {
		ctx.say(
			`→ writing per-repo IDE rule files into ${process.cwd()} (--with-init)`,
		);
		if (await runInit(ctx))
			ctx.results.installed.push(`caveman-init (${process.cwd()})`);
		else
			ctx.results.failed.push([
				"caveman-init",
				"src/tools/caveman-init.js failed",
			]);
		process.stdout.write("\n");
	...
	if (ctx.results.skipped.length) {
		...
		for (const [id, why] of ctx.results.skipped)
			process.stdout.write(`    • ${id} — ${why}\n`);
	}
	if (ctx.results.failed.length) {
		ctx.warn("  failed:");
		for (const [id, why] of ctx.results.failed)
			process.stderr.write(`    • ${id} — ${why}\n`);
	}
	if (
		...
		process.stdout.write(
			"  nothing detected. run with --list to see all 30+ supported agents,\n",
		);
		process.stdout.write(
			"  or pass --only <agent> to force a specific target.\n",
		);
	}
	process.stdout.write("\n");
	ctx.note(
